...
// NOLINTNEXTLINE(cppcoreguidelines-macro-usage)
#define LOG_AND_THROW_BLOB_EXCEPTION_NO_ERRNO(message) \
    { \
        VLOG_LP(log_info) << (message); \
        throw limestone_blob_exception(exception_type::blob_error, std::string((message)) + " (at " + extract_filename(__FILE__) + ":" + std::to_string(__LINE__) + ")", 0); \
    }
