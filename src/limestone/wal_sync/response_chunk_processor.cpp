...
void response_chunk_processor::handle_response(limestone::grpc::proto::GetObjectResponse const& response) {
    ...
    boost::filesystem::path object_rel_path{object_proto.path()};
    auto validate_relative_path = [this, &object_proto](boost::filesystem::path const& rel_path, transfer_state* state) -> bool {
        if (rel_path.empty()) {
            set_failure("object path is empty for object_id: " + object_proto.object_id(), state);
            return false;
        }
    ...
    if (it == states_.end()) {
        ...
        if (!validate_relative_path(object_rel_path, nullptr)) {
            return;
        }
        transfer_state state{};
        state.final_path = base_dir_ / object_rel_path;
    ...
    if (!state.saw_first_chunk) {
        ...
        if (!validate_relative_path(object_rel_path, &state)) {
            return;
        }
        boost::filesystem::path expected_path = base_dir_ / object_rel_path;
    ...
    auto expected_offset = static_cast<std::uint64_t>(state.received_bytes);
