...
* LOG-1 レベルなら問題ない。(RocksDB だと) 吸い上げ機構が Writer として 1VDS の DB ディレクトリを更新し、本体側 limestone は restore 処理等で読む際には ReadOnlyDB / SecondaryDB として DB ディレクトリを読めばよい。
...
* VB から safe snapshot version までの範囲のログエントリを順に取り出し、1VDS に逐次データ反映する。

### 単バージョンデータストア (mono version datastore / 1VDS)

* RocksDB を使用する
    * RocksDB に 1VDS 用の Column Family を作り利用する (あるいは default CF を使用する)
* あるいは 永続化可能で堅牢な KVS を手で実装

(以下では、1VDS は RocksDB を使用するものと仮定して記述する)
...
* フルバックアップ処理:
    * (PITRと同様に) メインDB一時保存領域に DB領域(1VDS と VB) の一時コピーを作る。
...
* フルバックアップ復元処理
    * 外部バックアッププロセスから、バックアップ領域 (DB領域(1VDS と VB) 、PITR 保存領域) が、ハードリンク状態も含めて復元される。(上記処理の逆)
    * メインDB一時保存領域のデータを DB領域(1VDS と VB) に取り出し、復元する。
