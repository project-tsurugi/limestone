...
* この機構に対して、ログディレクトリを監視し、追記されたログエントリを VB に吸い上げる機構を外付けすることにより、疎結合なアーキテクチャとする。
...
* 監視方法は
    ...
    * 監視対象候補すべてを open し、ファイルを無限 loop で read する
* 吸い上げ型の変種として、外付け監視機構から追記を検出するのでなく、`log_channel` がログデータを受け取り、pWAL ファイルに追記したのちに吸い上げ機構へ通知するという変種が考えられる。(通知型)

(コメント) 吸い上げは別プロセスで実現しうるのか
* LOG-1 レベルなら問題ない。(RocksDB だと) 吸い上げ機構が Writer として SVDS の DB ディレクトリを更新し、本体側 limestone は restore 処理等で読む際には ReadOnlyDB / SecondaryDB として DB ディレクトリを読めばよい。
...
write version を優先度(wv が低いものが高優先度)とする Priority Queue (PQ) で実現する。
...
* VB から safe snapshot version までの範囲のログエントリを順に取り出し、SVDS に逐次データ反映する。

## Single Version Datastore (SVDS)

* RocksDB を使用する
    * RocksDB に SVDS 用の Column Family を作り利用する (あるいは default CF を使用する)
* あるいは 永続化可能で堅牢な KVS を手で実装

(以下では、SVDS は RocksDB を使用するものと仮定して記述する)
...
* PITR 作成処理:
    ...
    * PITR 保存領域に DB領域(SVDS と VB) のコピーを作る。
        ...
        * RocksDB の Checkpoint を 予約ディレクトリ内に作成する。
            * 一貫性のとれた RocksDB イメージが作成される。要領の大半は immutable な SSTables ファイルであり、RocksDB はこれらのコピーはハードリンクで済ませるため、低負荷で実行可能である。
...
* フルバックアップ処理:
    * (PITRと同様に) メインDB一時保存領域に DB領域(SVDS と VB) の一時コピーを作る。
...
* フルバックアップ復元処理
    * 外部バックアッププロセスから、バックアップ領域 (DB領域(SVDS と VB) 、PITR 保存領域) が、ハードリンク状態も含めて復元される。(上記処理の逆)
    * メインDB一時保存領域のデータを DB領域(SVDS と VB) に取り出し、復元する。
