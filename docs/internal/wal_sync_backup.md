...
* end_epoch > current_epoch_idの場合、エラー
* 起動時のdurable_epoch > begin_epochの場合、起動時にGCによりBLOBGが削除される可能性があるため、エラーとする。
* end_epoch ≠ 0の場合、epochファイルはバックアップをしよせず。クライアント側で指定Epochのエポックファイルを作成する（将来的に不要となる可能性あり）。
