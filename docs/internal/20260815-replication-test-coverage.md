...
| 11 | RDMA 制御チャネルのフレーム検証 (version / payload_size / partial フラグ / 型違い → FATAL、キャスト失敗も FATAL 化済みだが到達不能) | N/A (TCP はストリームでフレームなし) | ✅ C: replica_server_test (version / payload_size / partial フラグ / 型違いの FATAL 4 分岐の EXPECT_DEATH + malformed payload FATAL + 正常ルーティングで epoch 永続) + E2E: scenario_tcpless の連続フレーム | **[G6]** 対応済み (単位 J3)。シーケンス不一致の検査は 2026-08-16 に削除 (順序保証は rdma-comm-lib が担い、不一致は lib 側で FATAL)。型違いは 2026-08-16 に drop → FATAL 化 (drop+ACK は flush に永続化を偽らせるため)。キャスト失敗分岐も FATAL 化したが、factory 経由で到達不能な防御コードのためテスト対象外 |
