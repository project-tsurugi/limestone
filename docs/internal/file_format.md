...
コンパクション済みファイルのファイル名のセット。
コンパクション済みファイルはファイル名がcmpctで始まるファイルで、
PWALファイルから重複を取り除いたファイルである。

コンパクション済みのPWALのファイル名のセット。
このセットに含まれるPWALファイルは、コンパクション済みなので、
データベースの起動やリカバリには使用しなく、削除可能である。
